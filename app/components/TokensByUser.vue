  ...
  <UCard v-model="portal" variant="subtle">
      ...
      <div class="grid grid-cols-6 gap-3">
        <div v-for="(ts, index) in portal.logins" :key="index" class="col-span-2">
          <UTooltip :text="ago(new Date(ts).toLocaleString()) || 'not known'" arrow :content="{ side: 'right', sideOffset: 2 }">
            <UChip class="w-full" :color="aging(ts)" inset size="xs">
              <UBadge class="p-2 rounded-full justify-center text-neutral min-w-[120px] w-full" :color="aging(ts)" variant="soft">
                {{ portal.online[index] }}
              </UBadge>
            </UChip>
          </UTooltip>
    ...
    <template #footer>
      <div class="justify-self-end text-sm">
        <i>Visitors over the past 3 months</i>
      </div>
    </template>
  </UCard>
</template>
