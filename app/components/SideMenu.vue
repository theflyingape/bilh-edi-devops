<template>
  <USlideover title="Health Connect Integration Team" description="BILH IT Technology & Innovation service line" :close="{
    color: 'action',
    variant: 'outline',
    class: 'rounded-full'
  }">
    <template #body>
      <div v-if="status !== 'unauthenticated'" class="flex flex-nowrap justify-evenly gap-8">
        <UCard v-if="user.enabled" class="w-full">
          <template #header>
            <UUser :name=user.name :description="`(${user.id}) ${user.comment}`" />
          </template>
