/**
 *  Authored by Robert Hurst <rhurst@bilh.org>
 */
...
//const searchBar = new SearchBarAddon({ searchAddon: search })
const rows = 8, cols = 100
...
let xterm = new Terminal(options)

xterm.loadAddon(new Unicode11Addon())
xterm.loadAddon(fit)
//xterm.loadAddon(search)
//xterm.loadAddon(searchBar)

window.onresize = () => {
    if (!xterm) return
    //const ui = <HTMLStyleElement>document.getElementsByClassName('uk-switcher')[0]
    //const monitor = <HTMLDivElement>document.getElementById('monitor')

    //  console has fixed ROWSxCOLS display, adjust terminal within browser window
    //monitor.style.width = `${ui.clientWidth}px`
    xterm.options.fontSize = 20
    let xy = fit.proposeDimensions()
    //let w = Math.trunc(20 * xy.cols / cols)
    //let h = Math.trunc(20 * xy.rows / rows)
    //xterm.options.fontSize = h < w ? h : w
